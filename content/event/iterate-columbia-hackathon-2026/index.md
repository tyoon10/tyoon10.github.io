---
title: "AI Club x Iterate NYC Hackathon"
event: AI Club x Iterate NYC Hackathon
event_url: https://luma.com/dyzbn70z
location: "Columbia Business School"

summary: "300 builders. 12 hours. $10,000+ in prizes. Co-organized with Anthropic, ElevenLabs, and YC-backed startups."

date: 2026-02-14T08:30:00-05:00
date_end: 2026-02-14T20:30:00-05:00
all_day: true

authors:
  - admin

tags:
  - Enterprise AI
  - Agentic AI
  - Hackathon

links:
  - icon: book-open
    icon_pack: fas
    name: Read the Recap
    url: /post/iterate-columbia-hackathon-2026-recap/
  - icon: linkedin
    icon_pack: fab
    name: LinkedIn Recap
    url: https://www.linkedin.com/posts/taewan-yoon_valentines-day-morning-at-830-am-with-activity-7429222303411904512-n_Nt
  - icon: link
    icon_pack: fas
    name: Event Page
    url: https://luma.com/dyzbn70z
---

300 builders. 12 hours. $10,000+ in prizes. One room.

The CBS AI Club and Iterate brought together builders from across New York City for a day-long hackathon at Columbia Business School's Geffen Hall Board Room. 60+ teams submitted final projects spanning healthcare AI, financial tools, creative applications, developer productivity, and education platforms.

## What Happened

- 12-hour build sprint in Geffen Hall at Columbia Business School
- $10,000+ in total prizes — cash awards, AI service credits, and partner merchandise
- 250 Shake Shack burgers and 200 ChaHalo milk teas
- Access to cutting-edge tools and APIs from our partners
- Judging by a panel of investors, founders, and technologists

## Sponsors & Partners

**Anthropic** | **ElevenLabs** | **Lovable** | **Blaxel** (YC X25) | **Finny** (YC S24) | **Perle** | **BizCrush** | **White Circle**

## Who Showed Up

Developers, designers, data scientists, MBAs with product chops, grad students, startup founders, working professionals — strangers from all across NYC, all building together.
