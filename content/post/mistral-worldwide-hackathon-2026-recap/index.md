...
[Mistral's CEO Arthur Mensch has been explicit](https://www.bloomberg.com/news/articles/2026-02-18/mistral-ceo-says-ai-dominance-hinges-on-openness-not-geography) about their strategic bet: "The fight for AI supremacy is between open and closed systems, not geography." That philosophy shaped every aspect of this hackathon. Every model available to builders was released under the Apache 2.0 license — fully permissive, unrestricted commercial use. No API lock-in. No usage caps. Builders owned their stack from the first line of code.

The model lineup available at the hackathon reflected Mistral's latest family — the Mistral 3 series, launched in December 2025 in partnership with NVIDIA. The flagship, Mistral Large 3, is a Mixture-of-Experts architecture with 675 billion total parameters but only 41 billion active at inference — meaning frontier-level capability at a fraction of the compute cost. It handles a 256K token context window with native multimodal support across text and images. On NVIDIA's GB200 NVL72 hardware, it achieved a 10x performance gain over the prior generation.

But what made the hackathon technically interesting was the small model story. Ministral 3 — a family of nine dense models across 3B, 8B, and 14B parameters — can run on devices with as little as 4GB of VRAM using 4-bit quantization. Standard laptops. No cloud. No internet required. And then there's Devstral 2, Mistral's coding-specialized model, which outperforms larger competitors like Qwen 3 Coder at 24B parameters. For a hackathon, this means teams could prototype locally, iterate fast, and deploy without infrastructure dependencies.

In conversations with the Mistral team — Howard on communications, Param and Devon on developer relations — a consistent theme emerged. They were not positioning Mistral as a benchmark competitor to OpenAI or Google. They were positioning it as infrastructure for builders who want sovereignty over their AI stack. Howard put it directly: "Open source — because we want to empower the builders, giving them the technology in their hands." Mensch himself frames this as a safer bet for any country building AI infrastructure locally — it accommodates cultural nuances, deploys on local hardware, and avoids dependence on centralized proprietary systems.
