...
date: 2026-06-21
description: "Finance spent twenty years industrializing its data and almost no time on its knowledge. Then AI changed the economics overnight. Drawing on my years as a practitioner, here is my framework: the data, the value, the constraints, and the stakeholders who decide whether any of it works."
featured: true
tags:
...
Early in my career, I owned the data room for a client company selling a stake to a private equity firm. This was before AI touched any of it. Due diligence was people and time: the buyer's counsel sent request lists, and I worked them by hand, finding the document, the contract, the board minute, the clause that settled each question, then deciding what the buyer was cleared to see before it crossed into the room. That last call was the part that stayed with me: you can hold material information and still be barred from acting on it.

A private-markets deal ran on that kind of manual retrieval. Teams of associates and lawyers read every page, because no one could afford to miss the one indemnity buried in an amendment to a schedule to an exhibit. The work was slow and expensive for a reason that now looks like the whole point: turning a company's history into something a buyer could act on meant paying humans to read it, tag it, and trace every figure back to its source. Knowledge got extracted one set of eyes at a time.

And yet the deal almost never turned on the volume of what we handed over. It turned on a handful of facts, and on one thing the data room rarely held: *why* the company's management had made the decisions they made. The contract told the buyer what was agreed. The financials told them what happened. The reasoning behind both, the judgment that produced the numbers, lived in people's heads and in email threads no one had filed.

That gap, the missing *why*, is the problem I now find most interesting in finance, and it has a name we rarely use for it: **knowledge**. Finance spent twenty years industrializing its *data* and almost no time industrializing its *knowledge*, and AI just changed the economics of that overnight. This is how I think about it. There are four parts, and the order matters: **data, value, constraints, and the part that decides everything, stakeholders.**
